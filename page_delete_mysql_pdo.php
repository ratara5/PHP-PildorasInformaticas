...
<head>
    ...
    <title>Delete Mysql PDO</title>
</head>
<body>
    <?php
        $search_name=$_GET["inp_name"]; //From form_delete_for_pdo.php
        try{
            ...
            $sql="DELETE FROM listacsvariables WHERE Nombre=:s_name;"; //with markups: replace '?' with ':name' 
            $set=$base->prepare($sql); //Type PDOStatement

            $set->execute(array("s_name"=>$search_name));

            echo "Se ha(n) borrado ".$set->rowCount()." registro(s) exitosamente<br>
                    ".$search_name;
